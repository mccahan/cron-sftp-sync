...
RUN echo '#!/bin/sh' > /app/sync.sh && \
    ...
    echo '  lftp -u "$USER","$PASS" -e "set xfer:use-temp-file yes; set xfer:temp-file-name *.lftp; mirror --continue --parallel=2 $REMOTE_PATH $LOCAL_PATH; bye" -p $PORT $PROTO://$SERVER' >> /app/sync.sh && \
